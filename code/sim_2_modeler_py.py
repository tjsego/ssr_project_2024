...
class SimulationReport:
    ...
    def generate_metadata(self) -> sim_lib.Metadata:
        ...
        param_dists = [sim_lib.ParamDist(param_name=n, dist_name=t[0], dist_params=t[1])
                       for n, t in self.dists.items()]
        return sim_lib.Metadata(sample_size=sample_size,
                                simulator='deterministic',
                                ks_stat_mean=float(np.mean(self.ks_stats_samp_hist[-1])),
                                ks_stat_stdev=float(np.std(self.ks_stats_samp_hist[-1], ddof=1)),
                                ks_stat_sz=len(self.ks_stats_samp_hist[-1]),
    ...
    def simulate(model_string: str,
        ...
        report = SimulationReport(model_string=model_string,
                                  ...
                                  results={name: np.zeros((num_samples_incr, num_steps), dtype=float)
        ...
        idx_current = 0
        iter_current = 0
        err_current = min(1.0, err_thresh + 1)

        while err_current >= err_thresh:

            if iter_current > 0:
                print(
                    f'Iteration {iter_current} ({idx_current}): {report.stat_hist[-1][1]}, {report.stat_hist[-1][2]} ({err_thresh})')
            else:
            ...
            for res in sim_lib.exec_rr_batch(num_samples_incr_curr, rr, t_fin, num_steps, stochastic, dists=dists):
                for name in var_names:
                    report.results[name][idx_current+idx_received, :] = sim_lib.round_arr_to_sigfigs(
                        extract_rr_results(res, name), report.sig_figs
                    )
                idx_received += 1
            ...
            report.stat_hist.append(
                (idx_current, np.average(report.ks_stats_samp_hist[-1]), np.std(report.ks_stats_samp_hist[-1]))
            )
