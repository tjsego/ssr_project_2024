...
class ComparisonResult:
    ...
    def __init__(self, sample_size: int, exec_start=True):
        ...
        if exec_start:
            ...
            self.self_sim = np.average(self.self_sim_evals), np.std(self.self_sim_evals)

            results_half = {k: v[:int(sample_size/2), :] for k, v in self.results.items()}
            ...
            for f in beta_facts:
                ...
                rr = sim_lib.make_rr(model_string, stochastic)
                results_comp = {n: np.zeros((sample_size, num_steps)) for n in var_names}
                for j, res in enumerate(sim_lib.exec_rr_batch(sample_size, rr, t_fin, num_steps, stochastic, dists=dist_info)):
                    for name in var_names:
    ...
    def from_json(cls, data):
        ...
        result.dist_info = {k: (v[0], (float(v[1][0]), float(v[1][1]))) for k, v in data['dist_info'].items()}
